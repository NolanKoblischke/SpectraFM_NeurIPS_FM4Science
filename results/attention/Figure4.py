...
def plot_all_attention(attention, wavelength, stellar_types, directory, spectra):
    fig, (ax1, ax2) = plt.subplots(2, 1, figsize=(40, 30), sharex=True)
    ...
    for idx, (stellar_type, ax) in enumerate(zip(stellar_types, [ax1, ax2])):
        ...
        ax.plot(wavelength, 1.8*spectra[stellar_type] + 3.6, linewidth=5, alpha=1.0, rasterized=True)
        ...
        for index, row in spectral_lines.iterrows():
            ...
            if stellar_type == "dwarves":
                ...
                if row['Element'] in ['O', 'Si', 'Mg']:
                    y_pos = -1.2
                ax.text(row['Wavelength'], y_pos, row['Element'], rotation=0, va='top', ha='center', fontsize=50)
        ax.set_ylim(-0.5, len(properties) * 1.0 + 1.0)
        ax.tick_params(axis='both', which='major', labelsize=70, width=3, length=20)
        for spine in ax.spines.values():
            spine.set_linewidth(4)
    
    # Set overall plot labels and styling
    fig.suptitle("SpectraFM Attention (Inverted)", fontsize=90)
    fig.text(0.5, -0.019, "Wavelength (Angstroms)", ha='center', fontsize=90)
    fig.text(-0.02, 0.5, "Normalized Attention", va='center', rotation='vertical', fontsize=90)
